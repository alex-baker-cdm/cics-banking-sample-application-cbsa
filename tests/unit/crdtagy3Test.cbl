      ******************************************************************
      * crdtagy3Test - isolated unit test for CRDTAGY3 (dummy credit
      * agency #3) running off-CICS through the GnuCOBOL shim harness.
      *
      * CRDTAGY3 is byte-for-byte near-identical to CRDTAGY1; the only
      * behavioural difference is the container name it reads/writes:
      * CIPC (CRDTAGY1 uses CIPA) on the same channel CIPCREDCHANN.
      ...
      ******************************************************************
       ...
       PROGRAM-ID. CRDTAGY3TEST.

       DATA DIVISION.
       WORKING-STORAGE SECTION.
      * Container layout - must match WS-CONT-IN inside CRDTAGY3.
       01 WS-CONT-IN.
       ...
       01 WS-CONTAINER-NAME             PIC X(16) VALUE 'CIPC'.
       ...
       PROCEDURE DIVISION.
       MAIN-A.
           DISPLAY "=== crdtagy3Test : CRDTAGY3 (credit agency) ==="
           ...
           IF WS-FAILURES = 0
              DISPLAY "RESULT: crdtagy3Test PASSED"
              MOVE 0 TO RETURN-CODE
           ELSE
              DISPLAY "RESULT: crdtagy3Test FAILED (" WS-FAILURES ")"
              MOVE 1 TO RETURN-CODE
           END-IF

           STOP RUN.

      * Seed the input container, run CRDTAGY3, read the result back.
       RUN-ONCE.
           INITIALIZE WS-CONT-IN
           MOVE "CIPC"       TO WS-CONT-IN-EYECATCHER
