      ******************************************************************
      * crdtagy2Test - isolated unit test for CRDTAGY2 (dummy credit
      * agency #2) running off-CICS through the GnuCOBOL shim harness.
      *
      * CRDTAGY2 is byte-for-byte near-identical to CRDTAGY1; the only
      * behavioural difference is the container name it reads/writes:
      * CIPB (CRDTAGY1 uses CIPA) on the same channel CIPCREDCHANN.
      * See tests/harness/README.md ("CRDTAGY family") for the full list
      * of byte-level differences between the five agencies.
      *
      * Strategy: seed the CIPB container via CICSCONT, invoke CRDTAGY2
      * (GET container -> compute score -> PUT container), then GET the
      * container back to inspect the score. CRDTAGY2 seeds RANDOM from
      ...
      ******************************************************************
       ...
       PROGRAM-ID. CRDTAGY2TEST.

       DATA DIVISION.
       WORKING-STORAGE SECTION.
      * Container layout - must match WS-CONT-IN inside CRDTAGY2.
       01 WS-CONT-IN.
       ...
       01 WS-CONTAINER-NAME             PIC X(16) VALUE 'CIPB'.
       ...
       PROCEDURE DIVISION.
       MAIN-A.
           DISPLAY "=== crdtagy2Test : CRDTAGY2 (credit agency) ==="
           ...
           IF WS-FAILURES = 0
              DISPLAY "RESULT: crdtagy2Test PASSED"
              MOVE 0 TO RETURN-CODE
           ELSE
              DISPLAY "RESULT: crdtagy2Test FAILED (" WS-FAILURES ")"
              MOVE 1 TO RETURN-CODE
           END-IF

           STOP RUN.

      * Seed the input container, run CRDTAGY2, read the result back.
       RUN-ONCE.
           INITIALIZE WS-CONT-IN
           MOVE "CIPB"       TO WS-CONT-IN-EYECATCHER
