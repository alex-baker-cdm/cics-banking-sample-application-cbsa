      ******************************************************************
      * crdtagy4Test - isolated unit test for CRDTAGY4 (dummy credit
      * agency #4) running off-CICS through the GnuCOBOL shim harness.
      *
      * CRDTAGY4 is byte-for-byte near-identical to CRDTAGY1; the only
      * behavioural difference is the container name it reads/writes:
      * CIPD (CRDTAGY1 uses CIPA) on the same channel CIPCREDCHANN.
      * See tests/harness/README.md ("CRDTAGY family") for the full list
      * of byte-level differences between the five agencies.
      *
      * Strategy: seed the CIPD container via CICSCONT, invoke CRDTAGY4
      * (GET container -> compute score -> PUT container), then GET the
      * container back to inspect the score. CRDTAGY4 seeds RANDOM from
      ...
      ******************************************************************
       ...
       PROGRAM-ID. CRDTAGY4TEST.

       DATA DIVISION.
       WORKING-STORAGE SECTION.
      * Container layout - must match WS-CONT-IN inside CRDTAGY4.
       01 WS-CONT-IN.
       ...
       01 WS-CONTAINER-NAME             PIC X(16) VALUE 'CIPD'.
       ...
       PROCEDURE DIVISION.
       MAIN-A.
           DISPLAY "=== crdtagy4Test : CRDTAGY4 (credit agency) ==="
           ...
           IF WS-FAILURES = 0
              DISPLAY "RESULT: crdtagy4Test PASSED"
              MOVE 0 TO RETURN-CODE
           ELSE
              DISPLAY "RESULT: crdtagy4Test FAILED (" WS-FAILURES ")"
              MOVE 1 TO RETURN-CODE
           END-IF

           STOP RUN.

      * Seed the input container, run CRDTAGY4, read the result back.
       RUN-ONCE.
           INITIALIZE WS-CONT-IN
           MOVE "CIPD"       TO WS-CONT-IN-EYECATCHER
