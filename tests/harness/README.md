...
| `CICSLINK` | `LINK PROGRAM(..) COMMAREA(..)`     | Generalized LINK double. `PROGRAM('INQCUST ')` returns a **valid customer** (`INQCUST-INQ-SUCCESS='Y'`); force not-found with `CBSA_TEST_INQCUST_SUCCESS=N` or the `LNKINQOK` driver op. Also emulates **`INQACCCU`** (fills the ODO account array from a driver-scripted account list) and **`DELACC`** (captures each account key passed so the driver can assert the cascade). See "Generalized LINK — `CICSLINK`" below. |
...
| `CICSENQ`  | `ENQ`, `DEQ`                        | No-op resource lock: always `resp=0`. Off-mainframe there is a single run unit, so serialization is unnecessary. |
...
`RUN TRANSID ... CHANNEL ... CHILD`, `FETCH ANY`, and the VSAM
file-control verbs `READ`, `READ UPDATE`, `REWRITE`, `WRITE`, `DELETE`,
`STARTBR`, `READNEXT`, `READPREV`, `ENDBR`. **`EXEC SQL`** against `ACCOUNT`
(SELECT / UPDATE / DELETE / cursor) and `INSERT INTO PROCTRAN` are also
supported (see the Db2 sections below). Other verbs (`ADDRESS`, `GETMAIN`,
